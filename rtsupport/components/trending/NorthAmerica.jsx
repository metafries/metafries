...
class NorthAmerica extends Component {
  ...
  async componentDidMount() {
    this.setState({
      opts: await this.props.getTotalOfContinent('NA')
    })
    let next = await this.props.getEventsByContinent('NA')
  ...
  loadMoreEvents = async() => {
    ...
    let next = await this.props.getEventsByContinent('NA', lastEvent)
    if (next && next.docs && next.docs.length <= 1) {
      this.setState({
        loader: false
      })
    ...
    return (
      <div>
        ...
        </h6>
        {
          ...
          <h6 className='info-text-box mb-3 mx-3 p-2'>
            <i class="fas fa-info-circle mr-2"></i>
            Haven't find any event located in North America.
          </h6>
        }
        <EventList 
          type={type}
