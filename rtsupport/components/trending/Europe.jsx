...
class Europe extends Component {
  ...
  async componentDidMount() {
    ...
    let next = await this.props.getEventsByContinent(selectedStatus, 'EU')
  ...
  loadMoreEvents = async() => {
    ...
    let next = await this.props.getEventsByContinent(selectedStatus, 'EU', lastEvent)
    if (next && next.docs && next.docs.length <= 1) {
      this.setState({
        loader: false
      })
  ...
  handleStatusChange = async(selectedStatus) => {
    ...
    let total = await this.props.getTotalOfContinent(selectedStatus, 'EU')
    this.setState({
      loader: false,
      initialize: true,
      loadedEvents: [],  
