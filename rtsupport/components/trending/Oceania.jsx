...
class Oceania extends Component {
  ...
  async componentDidMount() {
    ...
    let next = await this.props.getEventsByContinent(selectedStatus, 'OC')
  ...
  loadMoreEvents = async() => {
    ...
    let next = await this.props.getEventsByContinent(selectedStatus, 'OC', lastEvent)
    if (next && next.docs && next.docs.length <= 1) {
      this.setState({
        loader: false
      })
  ...
  handleStatusChange = async(selectedStatus) => {
    ...
    let total = await this.props.getTotalOfContinent(selectedStatus, 'OC')
    this.setState({
      loader: false,
      initialize: true,
      loadedEvents: [],  
