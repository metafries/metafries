...
class Antarctica extends Component {
  ...
  loadMoreEvents = async() => {
    ...
    let next = await this.props.getEventsByContinent('AN', lastEvent)
    if (next && next.docs && next.docs.length <= 1) {
      this.setState({
        loader: false
      })
    ...
    return (
      <div>
        ...
        <h6 className='info-text-box mb-3 mx-3 p-2'>
          ...
          The searching results are now limited to Antarctica.
        </h6>
        {
          opts === 0 &&
          <h6 className='info-text-box mb-3 mx-3 p-2'>
            <i class="fas fa-info-circle mr-2"></i>
            Haven't find any event located in Antarctica.
          </h6>
        }
        <EventList 
          type={type}
